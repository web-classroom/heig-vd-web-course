...
find . -type f -name "COURSE_MATERIAL.md" -mindepth 3 -maxdepth 2 -exec sh -c ' \
    sed "/<details>/,/<\/details>/d" "$1" | pandoc --from=gfm --to=html --standalone --output "${1%.md}.html" \
' sh {} \;

# Convert HTML files to PDF
find . -type f -name "COURSE_MATERIAL.html" -mindepth 3 -maxdepth 2 -exec sh -c ' \
...
find . -type f -name "COURSE_MATERIAL.pdf" -mindepth 3 -maxdepth 2 -exec sh -c ' \
    mv "$1" "$(dirname "$1")/$(basename "$(dirname "$1")")-course-material.pdf" \
' sh {} \;

# Delete all HTML files
find .  -type f -name "COURSE_MATERIAL.html" -mindepth 3 -maxdepth 2 -delete
